...
public class PurchaseOrderController {
    ...
    public Map<String, List<PurchaseOrder>> getOrdersWithCriteria(@RequestParam Map<String, String> query) {
        ...
        // possible scenarions:-
        //  1) both supplier and status are specified.(only one supplier and its list of orders)
        //  2) neither of them are specified.(multiple suppliers with a list of orders for each of them)
        //  3) only supplier is mentioned.(only one supplier and its list of orders)
        //  4) only status is mentioned.(multiple suppliers with a list of orders)
        // this is going to be long, sorry!

        // 1)
        if (query.containsKey("supplier") && query.containsKey("status")) {
            List<PurchaseOrder> orders = purchaseOrderService.getOrdersOfSpecificSupplierUnderSpecificStatus(query.get("supplier"), query.get("status"));
        ...
        // 4)
            ...
            // here we get the response frmo the service layer already tucked inside a map,yey!
