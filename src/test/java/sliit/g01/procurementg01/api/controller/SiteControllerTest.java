...
import sliit.g01.procurementg01.api.utility.SiteUtils;
...
public class SiteControllerTest extends SiteUtils {
	...
	public void retrieveSiteDataBySiteId_Test() throws Exception {
		...
		mvc.perform(get("/site/" + siteId).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void deleteSiteData_Test() throws Exception {
		...
		mvc.perform(delete("/site/" + siteId).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}
	...
	public void retrieveAllSiteData_Test() throws Exception {
		...
		mvc.perform(get("/site/").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void saveSiteDataException_Test() throws Exception {
		mvc.perform(post("/site/add-new-site").content(ADD_NEW_SITE_JSON).contentType(APPLICATION_JSON_UTF8))
	...
	public void updateSiteData_Test() throws Exception {
		...
		MockHttpServletRequestBuilder builder = MockMvcRequestBuilders.put("/site/" + siteId)
				.contentType(APPLICATION_JSON_UTF8).accept(APPLICATION_JSON_UTF8).characterEncoding("UTF-8")
				.content(ADD_NEW_SITE_JSON);
